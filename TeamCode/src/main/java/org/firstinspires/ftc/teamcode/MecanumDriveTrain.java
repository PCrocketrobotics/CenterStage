...
MecanumDriveTrain {
    ...
    static final double         P_DRIVE_COEFF          = 0.05; //Originally 0.15
    ...
    public void imuTurnGlobal(double targetangle, boolean eBrake){
        ...
        while ((currentangle > maxerror || currentangle < minerror) && !eBrake){
            ...
            if (currentangle > targetangle){
                robot.mecanumDriveTrain.left_front.setPower(.4  * throttle);
                robot.mecanumDriveTrain.left_back.setPower(.4    * throttle);
                robot.mecanumDriveTrain.right_front.setPower(-.4 * throttle);
                robot.mecanumDriveTrain.right_back.setPower(-.4  * throttle);
            }
            if (currentangle < targetangle){
                robot.mecanumDriveTrain.left_front.setPower(-.4 * throttle);
                robot.mecanumDriveTrain.left_back.setPower(-.4 * throttle);
    ...
    public void gyroDrive(double left_front_distance, double left_back_distance,
        ...
        // Ensure that the opmode is still active
            ...
            // start motion.
            motor_power = Range.clip(Math.abs(motor_power), 0.0, .15);
            left_front.setPower(motor_power);
            right_front.setPower(motor_power);
            left_back.setPower(motor_power);
            right_back.setPower(motor_power);
